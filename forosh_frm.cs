...
{
    public partial class forosh_frm : Form
    {
        public forosh_frm()
        {
            InitializeComponent();
        ...
        private void forosh_frm_Load(object sender, EventArgs e)
        {
        ...
        private void pic_closee_Click(object sender, EventArgs e)
        {
            this.Close();
        }
        ...
        {
            if (int.Parse(txt_kala_tedad.Text) > int.Parse(txt_kala_mojodi.Text))
            {
                MessageBox.Show("تعداد خرید از موجودی انبار بیشتر است", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txt_kala_tedad.Clear();
                txt_kala_tedad.Focus();
                return;
            }
        ...
        {
            // کم کردن تعداد فروش از موجودی دیتابیس
            for (int i = 0; i < dataGridView1.RowCount - 1; i++)
            {
                DataTable dt = kalaTableTableAdapter.GetKala(int.Parse(dataGridView1.Rows[i].Cells[0].Value.ToString()));
                if (dt.Rows.Count > 0)
                {
                    // مقدار فروخته شده را از موجودی کم کنید
                    int foroshTedad = int.Parse(dataGridView1.Rows[i].Cells[3].Value.ToString());

                    // به روز رسانی موجودی در دیتابیس
                    int k = kalaTableTableAdapter.UpdateMojodi(foroshTedad, int.Parse(dataGridView1.Rows[i].Cells[0].Value.ToString()));
            ...
            for (int i = 0; i < dataGridView1.RowCount - 1; i++)
            {
                ...
                if (int.TryParse(dataGridView1.Rows[i].Cells[3].Value.ToString(), out kalaTedad) &&
                    int.TryParse(dataGridView1.Rows[i].Cells[2].Value.ToString(), out kalaPrice))
                {
                    int k = foroshFactorTableAdapter.InsertForoshFactor(
        ...
        private void btn_show_mosh_Click(object sender, EventArgs e)
        {
            moshtari_report_frm mrf = new moshtari_report_frm();
            mrf.ShowDialog();
        }

        private void pic_show_mosh_Click(object sender, EventArgs e)
        {
            btn_show_mosh_Click(null, null);
        }

        private void btn_print_Click(object sender, EventArgs e)
        {
            DGVPrinter print = new DGVPrinter();
            print.Title = "فاکتور فروش";
            print.SubTitle = String.Format("محصولات فروخته شده", print.SubTitleColor = Color.Black, print);
        ...
        private void txt_jam_kol_TextChanged(object sender, EventArgs e)
        {
            //if (txt_jam_kol.Text != "" || txt_jam_kol.Text != "0")              
            //{
            //    decimal price;
            //    price = decimal.Parse(txt_jam_kol.Text, System.Globalization.NumberStyles.Currency);
            //    txt_jam_kol.Text = price.ToString("#,#");
            //    txt_jam_kol.SelectionStart = txt_jam_kol.Text.Length;
            //}
        }
    }
}
